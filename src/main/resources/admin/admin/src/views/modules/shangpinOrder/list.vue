        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/kexinjuanzengxitong/upload/shangpinOrderMuBan.xls"
                        >批量导入商品订单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shangpinOrder','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shangpinOrderCourierNumber"
                                   header-align="center"
                                   label="快递单号">
                        <template slot-scope="scope">
            ...
            <el-button v-if="isAuth('shangpinOrder','订单') && scope.row.shangpinOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">退款</el-button>
            <el-button v-if="isAuth('shangpinOrder','订单') && scope.row.shangpinOrderTypes == 104 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
...
&& sessionTable=='users'" type="primary" icon="el-icon-reading" size="mini"
            ...
            >发货</el-button>
            <el-button v-if="isAuth('shangpinOrder','订单') && scope.row.shangpinOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">收货</el-button>
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                ...
                //本表字段
                     ...
                     '购买数量': "buyNumber",
                     '积分': "shangpinOrderTruePrice",
                     '快递公司': "shangpinOrderCourierName",
                     '快递单号': "shangpinOrderCourierNumber",
        ...
        methods: {
			...
			deliver(){
				...
				_this.$confirm(`确定要发货吗?`, "提示", {
					...
					_this.$http({
						url: "shangpinOrder/deliver?id="+id+"&shangpinOrderCourierNumber="+_this.shangpinOrderCourierNumber+"&shangpinOrderCourierName="+_this.shangpinOrderCourierName,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
